...
BallTrackerModule::BallTrackerModule():
  ns_socket(NULL),
  cmd_socket(NULL),
  debug(0),
  cliParam(0)
{
  const char* const channel[2] = {"readVideo", "writeCommand"};
  cliParam = new Param();
  context = new zmq::context_t(1);
  pthread_mutex_init(&cmdMutex, NULL);
  pthread_create(&cmdThread, 0, readCommandsHelper, this);
  portVideoIn.sock = new zmq::socket_t(*context, ZMQ_REP);
  portVideoIn.ready = true;
  zmq_sockets.push_back(&portVideoIn);
  portCommandOut.sock = new zmq::socket_t(*context, ZMQ_REQ);
  portCommandOut.ready = true;
  zmq_sockets.push_back(&portCommandOut);
}

BallTrackerModule::~BallTrackerModule() {
  delete cliParam;
  delete portVideoIn.sock;
  delete portCommandOut.sock;
}

void BallTrackerModule::Init(std::string & name) {
  ...
  
  {
    ...
    zmqPortName = "/balltrackermodule" + cliParam->module_id + "/video";
    portVideoIn.name = zmqPortName;
    zmqPortName = "/resolve" + portVideoIn.name;
    pns_record record;
    ...
    std::cout << "Bind to socket " << zmqPortName << std::endl;
    portVideoIn.sock->bind(zmqPortName.c_str());
  }
  {
    // outgoing port, function as server
    zmqPortName = "/balltrackermodule" + cliParam->module_id + "/command";
    portCommandOut.name = zmqPortName;
    zmqPortName = "/resolve" + portCommandOut.name;
    pns_record record;
    record.name = zmqPortName;
    zmqSs.clear(); zmqSs.str("");
    zmqSs << getpid();
...
void BallTrackerModule::readCommands() {
  while (true) {
    HandleCommand();
  }
}
...
void BallTrackerModule::Resolve(pns_record & record) {
...
void BallTrackerModule::SendAck(zmq::socket_t *s, bool state) {
  if (debug) std::cout << "Send ACK" << std::endl;
  SendRequest(s, state, true, "ACK");
}

bool BallTrackerModule::ReceiveAck(zmq::socket_t *s, bool & state, bool blocking) {
  int reply_size = 0;
...
char* BallTrackerModule::GetReply(zmq::socket_t *s, bool & state, bool blocking, int & reply_size) {
  if (s == NULL)
    return NULL;
  zmq::message_t reply;
  char* result = NULL;
...
void BallTrackerModule::SendRequest(zmq::socket_t *s, bool & state, bool blocking, std::string str) {
  if (state) {
...
void BallTrackerModule::HandleCommand() {
  int reply_size = -1;
  bool state = false;
...
void BallTrackerModule::Connect(std::string source, std::string target) {
...
zmq::socket_t* BallTrackerModule::GetSocket(std::string name) {
...
long_seq* BallTrackerModule::readVideo(bool blocking) {
  pthread_mutex_lock(&cmdMutex);
  int reply_size = -1;
  char *reply = GetReply(portVideoIn.sock, portVideoIn.ready, blocking, reply_size);
  if (!portVideoIn.ready) {
    delete [] reply;
    pthread_mutex_unlock(&cmdMutex);
    return NULL;
  }
  ...
  SendAck(portVideoIn.sock, portVideoIn.ready);
  pthread_mutex_unlock(&cmdMutex);
  if (reply_size < 1) {
  ...
  while (std::getline(ss, item, delim)) {
    ...
    ssItem << item;
    ssItem >> itemVal;
    portVideoValue.push_back(itemVal);
  }
  delete [] reply;
  return &portVideoValue;
}

bool BallTrackerModule::writeCommand(const std::string cmd) {
  std::stringstream ss; ss.clear(); ss.str("");
  ss << cmd; // very dirty, no endianness, etc, just use the stream operator itself
  pthread_mutex_lock(&cmdMutex);
  bool state = portCommandOut.ready;
  SendRequest(portCommandOut.sock, state, false, ss.str());
  if (state) portCommandOut.ready = false;
  if (!portCommandOut.ready) {
    bool ack_state = true;
    ReceiveAck(portCommandOut.sock, ack_state, true);
    if (ack_state) {
      portCommandOut.ready = true;
      pthread_mutex_unlock(&cmdMutex);
      return true;
