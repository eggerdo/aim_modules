...
#ifndef BALLTRACKERMODULE_H_
#define BALLTRACKERMODULE_H_
...
private:
  ...
  long_seq portVideoValue;
  zmq_socket_ext portVideoIn;
  
  zmq_socket_ext portCommandOut;
  
  static void* readCommandsHelper(void* object) {
    ((BallTrackerModule*)object)->readCommands();
    return NULL;
  }
...
public:
  ...
  // Default destructor
  virtual ~BallTrackerModule();
  
  // Extend this with your own code, first call BallTrackerModule::Init(name);
  ...
  /**
   * The "readVideo" function receives stuff over a zeromq REP socket. It works as a client. It is better not
  ...
  long_seq *readVideo(bool blocking=false);
  
  /**
   * The "writeCommand" function sends stuff over a zeromq REQ socket. It works as a server. It cannot be blocking because this
